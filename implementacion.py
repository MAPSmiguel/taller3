...
#creo carpeta para guardar los resultados 


# # Esto carga un archivo de ejemplo real que trae la librería mientras migue crea la carpeta
# rutas = get_testdata_files("CT*.dcm") # me da rutas de la libreria como para iniciar el codigo
# archivos=[]
# for ruta in rutas:
#     # me evista error de permiso, solo intenta leer si la ruta es un archivo
#     if os.path.isfile(ruta):
#         try:
#             dataset=pydicom.dcmread(ruta) #lee la ruta esto es mientra migue pasa ls carpeta
#             if 'PixelData' in dataset : #verifica que el archivo tenga datos de imagen
#                 archivos.append(ruta) #agrega a la lista de archivos,
#         except pydicom.errors.InvalidDicomError:
#             # Si falla, el taller dice que debemos manejar el error (lo saltamos) 
#             print(f"Archivo sin imagen: {os.path.basename(ruta)}") 
#             continue
#     else:
#         print(f"Ruta no es un archivo") 

# for archivo in archivos: #recorre cada archivo en la lista de archivos
#     id_archivo = os.path.basename(archivo)
#      #sacamos el nombre para usar luego para guardar las imagenes
#     #el monitor pide guardar  l nombre de cada archivo y que debe incluir un identificador del estudio o del archivo de origen.
#     dataset = pydicom.dcmread(archivo) #lee el archivo DICOM 
#     matriz_px = dataset.pixel_array
#     #print(matriz_px.shape) #esto es para ver que se esta leyendo cada imagen, se puede quitar despues


#     #normalizar
#     imagenuint8=cv2.normalize(matriz_px,None,0,255,cv2.NORM_MINMAX).astype(np.uint8) 
#     #normaliza la imagen a un rango de 0 a 255 y la convierte a uint8


#     #Ecualizacion
#     im_ecualizada=cv2.equalizeHist(imagenuint8)
#     #print(im_ecualizada.shape)
#     #plt.hist(im_ecualizada.flatten(), bins=256) #para verificar si sirve., pero como son tanto archivos se traba el computador

#     #Deteccion de bordes

    
# # La imagen sale limpia porque solo quedan las estructuras más marcadas del DICOM.
# # Como la imagen es de baja resolucion el ruido es  muy grande. 
# # Usar 255 como límite dejar pasar solo la información importante.
#     bordes = cv2.Canny(im_ecualizada, 200, 255)
#     bordes_rgb = cv2.cvtColor(bordes, cv2.COLOR_GRAY2RGB) #decido de color gris
#     #plt.imshow(bordes_rgb)
#     #plt.show() #para verificar si sirve 
    
#     #guardo en mi carpeta 
#     cv2.imwrite(output_path + "/" + id_archivo + "_ecualizada.png", im_ecualizada)
#     cv2.imwrite(output_path + "/" + id_archivo + "_bordes.png", bordes_rgb)
